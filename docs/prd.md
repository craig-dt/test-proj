...
- `bytes`, `packets` and `dst_port` are non-negative integers written as plain ASCII digits only: no sign, no underscores, no exponent, no Unicode digits. Epoch timestamps likewise, with an optional `.` fraction.
